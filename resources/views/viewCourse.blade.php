...
@section('content')
    <div class="flex gap-5 my-4 p-10">
        <div class="w-1/4">
            ...
            <div class="bg-white rounded-lg border shadow p-8">
                <div class="p-2">
                    ...
                    <div class="flex gap-3 mt-3">
                        @if ($isAlreadyExist)
                            <button type="button" class="bg-green-800 px-3 py-2 rounded text-white disabled:bg-green-600 disabled:cursor-not-allowed" disabled>Already Joined</button>
                        @else
                        <form action="{{route('student.course.join')}}" method="POST">
                            @csrf
                            <input type="hidden" value="{{auth()->id()}}" name="user_id">
                            <input type="hidden" value="{{$item->id}}" name="course_id">
                            <button type="submit" class="bg-green-800 px-3 py-2 rounded text-white">Join Course</button>
                        </form>
                        @endif
